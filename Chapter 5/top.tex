...
Premium principles and risk measures map a loss distribution to a real number, for pricing or risk management. For example the loss aversion reserve \citep{choo2009loss} is a weighted average loss where weights are monotonic increasing with the percentile rank loss. Loss aversion reserves are in turn equivalent to distortion premiums \citep{wang1996pct} and spectral risks \citep{acerbi2002spectral}. Generalised premium principles based on weighted average losses are discussed in \citet{furman2008weighted}. \citet{gerber1985additive} discusses an alternative approach based on the certainty equivalent loss under utility theory, the exponential premium \citep{deprez1985convex} being a specific example. Other common premiums or risks are discussed in \citet{mcneil2005qrm} and \citet{young2004premium}, such as value-at-risk, conditional-tail-expectation, and the standard deviation premium. \citet{artzner1999cmr} discusses ``coherence" properties of a premium principle or risk measure, namely translation invariance, positive homogeneity, monotonicity, and subadditivity.

Existing premium principles and risk measures are mostly ``one-sided", focussing on large loss outcomes and adding a positive loading to the expected loss. In pricing, a positive loading avoids inevitable ruin in the long run. In risk management, a positive loading captures worse than expected losses, which are of main concern. For example, the loss aversion reserve (or distortion premium and spectral risk) magnifies higher loss percentiles. The standard deviation premium sets the loading as a positive multiple of standard deviation. Assuming a concave utility function, representing risk aversion, the certainty equivalent based premium exceeds the expected loss. The Dutch premium \citep{van1992dutch} also assumes a positive loading, based on a multiple of the expected excess loss.

A ``two sided" premium with a greater focus on smaller loss outcomes and potential negative loading is important for business reasons. In a competitive market, a negative loading may apply in the short term to increase business volume or avoid loss of business, the latter if other market players are also applying negative loadings. Hence conflicting considerations exist -- a positive loading is financially sustainable in the long term, however competitive pressure may force a negative loading to ensure short term survival. A negative loading may sustain for a longer term if the financial loss is offset by profit from other products with positive loading.

The tradeoff premium (ToP) is a new premium principle addressing ``two-sided" concerns highlighted in the previous paragraph. The ToP is a weighted average loss, with U-shaped weights increasing as loss outcomes deviate from a subjective ``loss appetite." Hence smaller losses below the loss appetite are weighted more heavily since they reduce the competitiveness of the premium. In addition higher weight is imposed on larger losses above the loss appetite, reflecting their adverse financial impact. U-shaped weights are consistent with cumulative prospect theory \citep{tversky1992apt}, where extreme outcomes (both positive and negative) are magnified and moderate outcomes are diminished.
...
Examples shown in this paper express the ToP as a two-sided generalisation of existing one-sided premiums or risks, including value--at--risk, conditional--tail--expectation and expected--maximal--loss \citep{choo2009loss}. The two-sided generalisation reflects the undesirability of both small and large loss outcomes, with the loss appetite controlling their relative representation in the ToP.

The equilibrium ToP corresponds to the loss appetite. Hence at equilibrium, the appetite for loss is equal to the premium collected. Undesired deviations of loss outcomes from the loss appetite represent premium surplus or shortfall. The equilibrium premium is shown to be a tail-magnified measure of central tendency, refining the mean and median measures. Hence for a right skewed distribution, the equilibrium premium exceeds the expected loss, and vice versa for a left skewed distribution.
...
The tradeoff premium (ToP) is a weighted premium \citep{furman2008weighted}.   ToP  generalises loss aversion reserves \citep{choo2009loss}, distortion premiums \citep{wang1996pct} and spectral risks \citep{acerbi2002spectral} by assigning higher penalty weights to loss outcomes further from the ``loss appetite" rather than, as in the case of ordinary loss aversion, to larger loss outcomes. Penalty weights forming the ToP are aligned with cumulative prospect theory \citep{tversky1992apt}, where extreme outcomes, relative to a reference point, are magnified and moderate outcomes are diminished. In addition, the ToP minimises pricing error undesired in a competitive market.

To explain the ToP, consider first the loss aversion reserve, equivalent to the distortion premium and spectral risk, associated with a random loss  $x\geq 0$ with distribution function $F$ and percentile rank random variable $u\equiv F(x)$.  Then $u$ is uniformly distributed on the unit interval and indicates ``loss severity," with $0$ being least severe and $1$ being most severe. Given an increasing aversion function $\phi\geq0$ integrating to one, the loss aversion reserve is
...
$$
where $V_u$ is the value--at--risk \citep{mcneil2005qrm} given sufficiency probability $u$.  The second and third expressions in \eref{dp} are  the distortion premium and spectral risk, respectively.

The loss aversion reserve \eref{dp} is a weighted average loss with higher weight on larger severities. The distortion premium is the expected loss computed using a distorted distribution $\Phi\circ F$ dominating the original distribution, noting $\Phi$ is convex. The spectral risk is a weighted average of loss percentiles where higher percentiles are weighted more heavily. Hence loss aversion reserves, distortion premiums and spectral risks are ``one-sided", assuming sole concern on larger severities. Concern is characterised  by $\phi$ reweighing initially  equally weighted loss percentiles or severities. \cite{choo2009loss} shows the equivalence of loss aversion reserves with distortion premiums and spectral risks.

Loss appetite is central to the ToP. To introduce the concept of loss appetite, loss aversion reserves (equivalently distortion premiums and spectral risks) initially consider zero loss appetite.  Then larger losses are always feared more and penalised more heavily. However a positive loss appetite may be relevant or even optimal in a business setting. In a competitive market, premiums may be deliberately reduced to win or prevent loss of market share. Premium reduction is achieved with a positive loss appetite. Penalty weights decrease up to the loss appetite, and increase thereafter. Maximum loss appetite occurs if  penalty weights always decrease with severity and contrasts with zero loss appetite where penalty weights always increase with severity. The aversion function re-distributes weights, in regions below and above loss appetite.
...
$$
Here $\ell$ is a subjectively specified loss appetite on the percentile rank scale, with $\ell=0$ indicating no appetite for losses and $\ell=1$ indicating a complete appetite for losses.  The deviation $u-\ell$ is standardised so that $\psi_\ell(0)=\psi_\ell(1)=1$ with linear behaviour over the segments $[0,\ell]$ and $[\ell,1]$.

Similar to loss severity $u$,  satiation error $\psi_\ell(u)$ is uniformly distributed over the unit interval. Satiation error is also uniform over severities below and above the loss appetite. Zero loss appetite, $\ell=0$,  reduces  satiation error to loss severity: $\psi_0(u)=u$.   Maximum loss appetite, $\ell=1$ yields satiation error $\psi_1(u)=1-u$.  The role and selection of the loss appetite parameter $\ell$ is further discussed below.

The Tradeoff Premium (ToP) is defined analogous to \eref{dp} by imposing an aversion function $\phi$  on satiation error $\psi_\ell(u)$, yielding
...
The ToP is a ``two-sided" generalisation of existing loss aversion reserves, noting weights $\phi_\ell(u)=\phi\{\psi_\ell(u)\}$ increase as loss severity $u$ deviates from loss appetite $\ell$, rather than from zero. The overall weight below and above the loss appetite are
$$
...
$$
where  the $x_i$ are $n$ independent copies of $x$.  With minimum and max loss appetite, $\ell=0,1$
$$
\top_0=\E\left\{\max_{i=1,\ldots,n}(x_i)\right\} \cq \top_1=\E\left\{\min_{i=1,\ldots,n}(x_i)\right\} \;.
$$
Further  for $0\leq \ell\leq 1$, $\top_0\leq \top_\ell \leq \top_1$.   If $n=1$ then $\phi(u)=1$  and for all $\ell$,
$$
\top_\ell=\ell\E(x|x\leq V_\ell)+(1-\ell)\E(x|x>V_\ell) = \E(x) \;.
$$
The following key properties of the ToP are inferred from the above example, and are formalised in later sections:
\bi
\i The ToP is formed by combining extreme views of expected losses below and above the loss appetite, via an ``aversion adjustment." The expected loss below the loss appetite is reduced, by assuming the expected minimum, while the expected loss above the loss appetite is raised, by assuming the expected maximum. Without aversion adjustment the ToP reduces to the expected loss.

\i Loss appetite $0\leq \ell\leq 1$ controls the size of the ToP. $\top_\ell$ is at maximum if  $\ell=0$, that is if there is no appetite for loss.  Increasing loss appetite reduces the ToP.

\i Relative left and right tail skewness about $\ell$ also affect the ToP, given the loss appetite.  Increasing the skewness of the right tail while keeping the expectation constant increases the expected maximum of losses above loss appetite, resulting in a higher ToP.

\i Increasing loss aversion by increasing $n$ in $\phi(u)=nu^{n-1}$ accentuates the impact of relative tail skewness on the ToP. Consider a right skewed loss distribution about $\ell$. Increasing $n$ has a greater impact on the expected maximum of losses above the loss appetite than the expected minimum of losses below the loss appetite, thus increasing the ToP overall. With neutrality $n=1$ or $\phi=1$, relative tail skewness are ignored and the ToP is always the expected loss.

\i The same aversion attitude applies to losses below and above the loss appetite. In this example both losses are adjusted by focussing on the expected maximum or minimum over $n$ copies.
...
In a competitive market, overpricing can lead to  loss of future business. Hence smaller loss outcomes are feared. Larger loss outcomes are also feared, due to underpricing and financial loss. Loss appetite $\ell$ and the associated value--at--risk $V_\ell$ sets the boundary between perceived ``small" and ``large" losses. A low loss appetite $\ell$ yields a higher proportion of large losses and vice versa. U-shaped penalty weights $\phi_\ell(u)$ models the undesirability of various severities. Given premium $\pi$,  then
...
The loss appetite parameter $\ell$ is central to the ToP and distinguishes it from the existing premiums.   As mentioned just after \eref{tradeoff}, zero loss appetite $\ell=0$ yields the usual distorted premium, whereas the dual is obtained with $\ell=1$, the maximum loss appetite.

Loss appetite $\ell$ is  subjectively selected  from the unit interval. The chosen $\ell$ is a neutrality, ``preference" or ``comfort" point  with no aversion: $\phi_\ell(\ell)=0$ and no distribution adjustment: $\Phi_\ell(\ell)=\ell$.  Aversion increases as  $|u-\ell|$ increases.
...
Higher loss appetite reduces $T_\ell$. Zero loss appetite $\ell=0$ and maximum loss appetite $\ell=1$ yield maximum and minimum ToPs, respectively. Higher loss appetite indicates higher tolerance of larger loss outcomes, hence a greater willingness to charge lower premiums.
...
\end{equation}
where $\cov$ and $\cor$  denotes the covariance and correlation, respectively. The  first equality holds since  $\E\{\phi_\ell(u)\}=\E\{\phi(u)\}=1$.   Thus $\top_\ell$ can be thought of as a standard deviation premium \citep{young2004premium} -- the actual premium is the mean $\mu_x$ plus a multiple of the standard deviation $\sigma_x$ of the loss.

With ToP the multiple is $\sigma_\phi\cor\{x,\phi_\ell(u)\}$. The multiple can be positive or negative and  depends on two  factors:
\bi
\i The aversion standard deviation $\sigma_\phi$, representing the overall aversion to pricing error. High $\sigma_\phi$ implies aversion increases dramatically when pricing error increases, while neutrality to pricing error implies $\phi=1$ and hence $\sigma_\phi=0$. The value $\sigma_\phi$ does not depend on the loss appetite $\ell$.

\i The loss appetite $\ell$, controlling the correlation term. Zero loss appetite $\ell=0$ maximises the multiple, while increasing loss appetite reduces the multiple. Maximum loss appetite $\ell=1$ yields a negative standard deviation multiple, since $\phi_1(u)=1-u$ is negatively correlated with $x$. Note $\ell$ affects ToP only through the standard deviation multiple.
\ei


\subsection{Linearity}

The standard deviation multiple $\sigma_\phi\cor\{x,\phi_\ell(u)\}$ in \eref{stddev} is invariant to location and scale changes in $x$. Hence the ToP is linear in the loss:
$$
\top_\ell(\alpha+\beta x)=\alpha+\beta\top_\ell(x) \cq 0\leq \ell\leq 1 \;,
$$
where $\alpha$ and $\beta\geq 0$ are constants and $\top_\ell(x)$ is the ToP of loss $x$. The linearity property corresponds to translation invariance and positive homogeneity properties of coherent risk measures \citep{artzner1999cmr}. Other properties of coherent risk measures are discussed below.


\subsection{Stochastic dominance and monotonicity}

Suppose loss $y$ has first order stochastic dominance over $x$, or the distribution function of $y$ is less than $x$ at all points. Then $\top_\ell(y) >\top_\ell(x)$  for any loss appetite $\ell$. Hence ``larger" loss random variables have higher ToP, given the same loss appetite. The proof follows from the second expression for the ToP in \eref{tradeoff}, noting $\Phi_\ell$ is the same for both $x$ and $y$.

The ToP also preserves statewise dominance, the monotonicity property of coherent risk measures. When loss $y$ exceeds $x$ over all states, $y$ also stochastically dominates $x$ thus $y$ has a larger ToP compared to $x$.


\subsection{Non-subadditivity}

Unlike coherent risk measures, the ToP is not sub-additive: the ToP of the sum of losses may exceed the sum of ToPs for each individual loss.
...
$$
For the ToP where loss appetite is not zero or one, both small and large severities are undesired, and are simultaneously diversified upon aggregation. The overall impact on the ToP depends on the loss appetite: low loss appetite implies large severities have dominant concern, resulting in subadditivity. Conversely high loss appetite indicates dominant concern on small severities, yielding a super-additive ToP.
...
$$
noting the ToP is weighted average of all value--at--risk $V_u$ with non-negative weights $\phi(u)$, based on the third expression for the ToP in \eref{tradeoff}.
...
An equilibrium occurs if $F(\top_\ell)=\ell$ or $\top_\ell=V_\ell$: that is if the ToP $\top_\ell$ is the $\ell$--value--at--risk corresponding to loss appetite $\ell$. Hence at equilibrium the probability of premium sufficiency equals the loss appetite.  A solution $\ell_*$ to $\top_\ell=V_\ell$  exists  since
$
(\top_0>V_0)\times (\top_1<V_1) = 1
$.
...
\end{equation}
where $\ell_0$  is an initial loss appetite and $\top_*\equiv \top_{\ell_*}$ denotes the equilibrium ToP and  $V_*\equiv V_{\ell_*}=\top_*$ is the value--at--risk corresponding to the equilibrium loss appetite.

The iteration  \eref{iteration} has a practical interpretation. Suppose $\ell<\ell_*$ implying $\top_{\ell}> \top_*=V_*>V_{\ell}$ where the first inequality applies since $\top_\ell$ is decreasing in $\ell$. Then the value--at--risk at loss appetite $\ell$ falls below the ToP, that is the amount the insurer is willing to lose is less than the premium collected. This ``disequilibrium" creates a tendency to increase loss appetite or reduce premium.  An analogous argument applies if $\ell$ is above $\ell_*$.  Inconsistency between the updated loss appetite and ToP results in further calculation, until convergence.
...
A special case of equilibrium pricing occurs with neutrality to pricing error, $\phi=1$.   In this case $\top_{\ell}=\mu_x$, the mean loss, for all $\ell$. The  equilibrium loss appetite is $\ell_*=F(\mu_x)$.

The equilibrium ToP is also equal to the mean loss if the distribution is symmetric, and $\ell_*=F(\mu_x)=0.5$. This result is established by  noting satiation error $\psi_{0.5}(u)$ and hence aversion weights $\phi_{0.5}(u)$ are always symmetric about $u=0.5$. In addition loss percentiles $V_u$ are also symmetric about $u=0.5$ for a symmetric distribution, therefore the ToP at $\ell=0.5$ achieves equilibrium: $\top_{0.5}=\E\{V_u\phi_{0.5}(u)\}=V_{0.5}=\mu_x$, completing the proof.



\subsection{Central tendency and impact of skewed loss distribution}

The equilibrium ToP is a measure of central tendency akin to the mean and median. The equilibrium ToP magnifies both tails of the loss distribution, and thus includes a positive loading over the mean loss for a right skewed distribution and vice versa.
...
Hence the equilibrium ToP equalises expected premium surplus $|\top_*-x|_+$ and premium shortfall $|x-\top_*|^+$.   The aversion adjustment $\phi_*$ magnifies  larger surpluses or shortfalls except in the case of  neutrality $\phi\equiv1$.
...
$$
\E\left( |\mu_x-x|^+ \right) = \E\left( |x-\mu_x|^+ \right) \cq \E\left(|x\leq m_x|^+\right)=\E\left(|x>m_x|^+\right) \;.
$$
where $m_x$ is the median of $x$. The mean loss equalises expected surplus and shortfall without aversion adjustment, that is assuming neutrality towards pricing error. The median loss equalises probabilities of surplus and shortfall regardless of their magnitude.

Hence $\top_*$ is a measure of central tendency similar to the mean and median. The equilibrium ToP generalises the mean by including an aversion adjustment weighting larger deviations more heavily -- both tails of the loss distribution are amplified. The result of amplification is discussed in below. For example if the aversion function is $\phi(v)=nv^{n-1}$ then weights increase as a power  of the deviation.
...
As mentioned above, disequilibrium arises if the ToP differs from the loss appetite: $\top_\ell\neq V_\ell$. Disequilibrium may exist, and persist, due to conscious decision. Suppose $\ell<\ell_*$ resulting in $\top_\ell>V_\ell$, or loss appetite is less than premium. Disequilibrium persists if a decision is made to set loss appetite less than premium, for example when underpricing and financial loss is of utmost concern. A change in loss appetite only occurs when new factors create pressure on disequilibrium to be reduced or eliminated, such as fear of overpricing and loss of future business. In this case the loss appetite increases, resulting in a lower ToP. The pressure and hence adjustment cease when equilibrium is achieved: $\top_\ell=V_\ell$.
...
$$
a weighted average of lower and upper loss percentiles. The corresponding loss aversion reserve or distortion premium, and the dual, are
$
V_\alpha$ and $V_{1-\alpha}$
respectively, the upper and lower ``value--at--risk" \citep{mcneil2005qrm}. Value--at--risk is an extreme percentile in either tail, whereas the ToP in this case combines extreme percentiles in both tails to reflect concerns towards smaller and larger severities. Hence call this ToP the ``two-sided value-at-risk."   In this example $\phi$ is not increasing, but it nevertheless highlights the two-sided feature of ToPs.


\item Suppose $\phi(v)=(v>\alpha)/(1-\alpha)$, the step function where $0\leq\alpha\leq 1$. Satiation errors below $\alpha$ are ignored while larger errors above $\alpha$ are magnified by the factor $1/(1-\alpha)$. The resulting ToP is a weighted average of small and large losses:
$$
...
or the ``two-sided expected--maximal--loss," noting the one-sided equivalents are the expected maximal or minimal loss.
 ...
 Construct another aversion function weighting positive integer values of $n$ in $nv^{n-1}$, using the Poisson distribution with mean $\lambda$ and truncating away zero. The resulting aversion function as
$$
...
$$
Therefore the aversion function is exponentially increasing at rate $\lambda$. The expected value of the $n$-parameter is
$$
...
$$
percentile rank versions of the Esscher premium \citep{van1989properties}, $\E(xe^{\lambda x})/\E(e^{\lambda x})$. In addition the ToP with loss appetite $0<\ell<1$ is
$$
...
$$
where the last two expressions are a weighted average of two-sided expected--maximal--losses using identical Poisson weights as those applied in generating the aversion function. Note the aversion weights $\phi_\ell(u)$ are decreasing exponentially up to $u=\ell$, and increasing exponentially thereafter.


\end{itemize}
\cite{choo2009loss} constructs other aversion functions based on weighted averages $w\phi_1+(1-w)\phi_2$ and compositions $(\Phi_1\circ\Phi_2)'$, where $\phi_1$ and $\phi_2$ are existing aversion functions and $\Phi_1$ and $\Phi_2$ are corresponding distortion operators such that $\Phi_1'=\phi_1$ and $\Phi_2'=\phi_2$.
...
The solid line in the top right panel in \fref{illustration} graphs standardized ToP $\top_*$ against loss appetite $\ell$. As noted in \sref{s_properties}, $\top_*$ is decreasing in $\ell$. The maximum $\top_\ell^*$ is $1.3$ at zero loss appetite, corresponding to the expected maximum loss over $5$ copies. The minimum $\top_\ell^*$ is $-0.8$ at maximum loss appetite, corresponding to the expected minimum. In addition $\top_*>0$ or equivalently $\top_\ell>\mu_x$, a positive premium loading, for loss appetites up to $0.75$, and $\top_\ell<\mu_x$ thereafter. The ToP exceeds the mean loss over most loss appetites in this case since the gamma loss distribution is right skewed, hence positive deviations from loss appetite have a greater impact on the ToP compared to negative deviations.

The standardized equilibrium ToP $\top_*^*$ is also shown in the top left panel as the intersection between $\top_*$ and standardized values-at-risk $(V_\ell-\mu_x)/\sigma$, the latter indicated by the dotted line. Note $\top_*^*>0$, hence the equilibrium ToP exceeds the mean loss. As noted in \sref{s_equilibrium}, a right skewed loss distribution yields an equilibrium ToP exceeding the mean loss, since aversion adjustment has a greater impact on the right tail compared to the left tail.
...
Finally the bottom right panel in \fref{illustration} shows the impact of increasing overall aversion to pricing error, by increasing $n$. The ToP and its equilibrium value are generally higher for larger $n$. The reason is increasing $n$ places greater weight on extreme loss outcomes (both large and small), and results in a higher equilibrium ToP for a right skewed loss distribution (\sref{s_equilibrium}). On the other hand the ToP only increases with $n$ when the loss appetite is small to moderate, where large loss outcomes have dominant impact compared to small loss outcomes. For a high loss appetite, higher $n$ reduces the ToP as the focus shifts to smaller loss outcomes.
...
\begin{figure}
  \begin{center}
    ...
    \end{tabular}
    \caption{The top left panel plots the standardized ToP $\top_*$ and standardized loss percentiles (or values at risk) against loss appetite $\ell$. The top right panel plots the modified loss density $f_\ell(x)$ and original density $f(x)$ against $x$. The bottom left and right panels plot $\top_*$ against $\ell$ for varying $\alpha$ and $n$, respectively. Equilibrium ToPs in the bottom panels are indicated by asterisks.}
    \label{illustration}
  \end{center}
\end{figure}
